...
/*   Updated: 2024/01/12 20:02:28 by andrefil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_player(mlx_key_data_t keyd, void *param)
{
	...
	data = (t_data *)param;
	data->posix->x_plyr = data->content->person_img->instances->x / TILE_SIZE;
	data->posix->y_plyr = data->content->person_img->instances->y / TILE_SIZE;
	x = data->posix->x_plyr;
	y = data->posix->y_plyr;
	if (keyd.key == MLX_KEY_W && keyd.action == MLX_PRESS
		&& data->map->matrix[y - 1][x] != '1')
		up_movement(data, x, y);
	if (keyd.key == MLX_KEY_A && keyd.action == MLX_PRESS
		&& data->map->matrix[y][x - 1] != '1')
		left_movement(data, x, y);
	if (keyd.key == MLX_KEY_S && keyd.action == MLX_PRESS
		&& data->map->matrix[y + 1][x] != '1')
		down_movement(data, x, y);
	if (keyd.key == MLX_KEY_D && keyd.action == MLX_PRESS
		&& data->map->matrix[y][x + 1] != '1')
		right_movement(data, x, y);
}

void	put_elements_map(t_data *data, int32_t cols, int32_t rows)
{
	print_floor(data, (WIDTH_IMG * cols), (HEIGHT_IMG * rows));
	if (data->map->matrix[rows][cols] == '1')
		print_wall(data, (WIDTH_IMG * cols), (HEIGHT_IMG * rows));
	if (data->map->matrix[rows][cols] == 'C')
		print_bone(data, (WIDTH_IMG * cols), (HEIGHT_IMG * rows));
	if (data->map->matrix[rows][cols] == 'P')
		print_player(data, (WIDTH_IMG * cols), (HEIGHT_IMG * rows));
	if (data->map->matrix[rows][cols] == 'E')
		print_exit(data, (WIDTH_IMG * cols), (HEIGHT_IMG * rows));
}
